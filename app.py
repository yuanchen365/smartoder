...
st.sidebar.markdown("---")
st.sidebar.subheader("3. 監控控制")

# 啟動按鈕
if st.sidebar.button("🚀 啟動監控", disabled=st.session_state.monitoring or not st.session_state.logged_in):
    # 準備監控目標 Targets
    ...
    if not targets:
        ...
        # 啟動背景執行緒
        # 注意：我們需要傳遞參數給 thread
        # 這邊參數需要從 Main Area 取得，因此先定義變數 (雖然在按鈕邏輯中引用外部變數不太好，但在 Streamlit Script 執行流中是可行的，只要確保變數已賦值)
        # 為了安全，我們將取得變數的邏輯往下移，或者在這裡使用 session state 暫存值?
        # 更好的做法：Streamlit 每次 rerun 都會執行整個 script。這段 code 在執行時，下方的 inputs 已經被定義了嗎？
        # 不一定。Streamlit 是由上而下執行。如果按鈕在 sidebar 上方，而 inputs 在 main area 下方...
        # 實際上 inputs (strategy params) 應該要在按鈕被按的時候讀取最新的值。
        pass 
        # 修正：將監控啟動邏輯放到 inputs 定義之後，或是改變排版順序。
        # 但 sidebar 必須在上面定義。
        # 解法：在 elif st.sidebar.button 中，我們只能讀取 session_state 或是 widget 的值。
        # 由於 inputs 是在下方定義的，我們可以用 st.session_state["widget_key"] 來存取。
        
        # 但為了避免複雜，我們保持原樣，只是要注意變數作用域。
        # 在 Streamlit 中，如果按鈕被按，整個 script 會重跑。
        # 我們將 "處理按鈕事件" 的邏輯放到 Script 最後面執行？ 
        # 不行，sidebar button 回傳 True 只有在該次 run。
        
        # 正確做法：將策略參數設定移到 Sidebar 或是確保在按鈕邏輯執行前，參數已有預設值。
        # 或是我們用一個 placeholder flag，在 script 末尾檢查 flag 並啟動 thread。
        pass

# 讓我們調整結構：先定義所有 UI 輸入，最後處理按鈕邏輯。
# ==========================================
